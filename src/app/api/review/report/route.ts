...
import { getCategoryPerformance } from "@/lib/intelligence/categories";
  ...
  try {
    ...
    for (const s of strategies ?? []) {
      ...
      strategyPerf.push({
        ...
        win_rate: trades.length > 0 ? Math.round((wins / trades.length) * 1000) / 10 : null,
    ...
    const categoryPerf = await getCategoryPerformance();
    report.categories = categoryPerf;
    ...
    report.overall = {
      ...
      win_rate: totalResolved > 0 ? Math.round((totalWins / totalResolved) * 1000) / 10 : null,
      total_pnl: Math.round(totalPnl * 100) / 100,
